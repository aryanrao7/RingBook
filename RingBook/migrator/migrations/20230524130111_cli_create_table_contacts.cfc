...
--->
<cfcomponent extends="wheels.migrator.Migration" hint="CLI create_table_contacts">
  <cffunction name="up">
  	<cfset hasError = false />
  	<cftransaction>
	    <cfscript>
	    	try{
				t = createTable(name='contacts', force='false', id='true', primaryKey='id'); 
				t.string(columnNames='firstname,lastname,email,phone,address', null = false, limit =45);
				t.blob(columnNames='profilePic',null = false,limit =45);
				t.int(columnNames="userID", null = false);
				t.timestamps();
				t.create();
	    	}
	    	catch (any ex){
